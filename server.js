...
app.get('/api/eleves/:annee', async (req, res) => {                    
    try {                                                                  
        ...
        if (ecole) {
            query = query.or(`nom_ecole.eq.${ecole},ecole.eq.${ecole}`);
        } else {
            const nomEcole = req.session.nomEcole;
            if (nomEcole) {
                query = query.eq('nom_ecole', nomEcole);
            }
        }

        const { data: eleves, error: elevesError } = await query.order('nom', { ascending: true });                                                                                               
    ...
    try {                                                                  
        ...
        for (const item of donnees) {
            ...
            upsertData.push({
                ...
                periode, 
                matricule,                                                                  
                graphisme: n.graphisme || null, 
                disc_visuelle: n.disc_visuelle || null, 
                exp_ecrite: n.exp_ecrite || null,
                copie: n.copie || null, 
                dictee: n.dictee || null, 
                ecriture: n.ecriture || null,
                exp_texte: n.exp_texte || null, 
                aem: n.aem || null, 
                math: n.math || null, 
                edhc: n.edhc || null,
                lecture: n.lecture || null, 
                dessin: n.dessin || null, 
                poesie: n.poesie || null,
                total: item.total || 0, 
                moyen: item.moyenne || 0, 
                rang: item.rang || 0, 
...
app.get('/api/drh/ecoles', async (req, res) => {
    try {
        const iepParam = req.query.iep || (req.session && req.session.iep);
        
        let query = supabase.from('configuration').select('nom_ecole, iepp');

        if (iepParam) {
            query = query.ilike('iepp', `%${iepParam.trim()}%`);
        }

        const { data, error } = await query;

        if (error) {
            console.error("Erreur Supabase écoles IEPP:", error);
            return res.json([]);
        }

        let ecolesSet = new Set();
        if (Array.isArray(data)) {
            data.forEach(item => {
                let nom = item.nom_ecole;
                let ieppEnregistre = item.iepp ? item.iepp.trim().toUpperCase() : '';
                
                // Si un paramètre d'IEP est spécifié, on filtre strictement pour éviter les correspondances partielles croisées (ex: ADIAKE vs sous-IEP)
                if (iepParam && ieppEnregistre !== iepParam.trim().toUpperCase()) {
                    return;
                }

                if (nom && typeof nom === 'string' && nom.trim() !== '') {
                    ecolesSet.add(nom.trim());
                }
            });
        }

        res.json([...ecolesSet].sort());
    } catch (err) {
