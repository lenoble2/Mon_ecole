...
app.post('/api/basculer-eleves', async (req, res) => {
    ...
    const client = await pool.connect(); // On récupère une connexion du pool pour la transaction
    try {
        await client.query('BEGIN');
        ...
        client.release(); // On libère la connexion ici !
    }
});
...
app.post('/api/update-notes', async (req, res) => {
    ...
    // On utilise une connexion dédiée du pool pour la transaction
    const client = await pool.connect();
    try {
        await client.query('BEGIN');
